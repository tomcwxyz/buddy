...
The track renderer now uses connected segments rather than placing every piece at `index × width`. `connectedTrackGeometryForKinds` carries a real endpoint and heading from one segment to the next. `bank-left` and `bank-right` change that heading, so every later piece is physically redirected. The builder renders each segment in its own translated/rotated SVG group and exposes the current construction endpoint as **Build here**.

Ride traversal also no longer infers the active piece from screen X. At launch time it measures the station path and each segment path, walks them by accumulated route distance, transforms local path coordinates back into world coordinates, and uses that actual segment index for speed checks, tunnel/launch/brake effects and stunt rotation. This is the foundation for more freeform spatial track pieces without breaking ride physics.

The connected-track model now also supports variable physical piece length through `lengthMultiplier` / `pieceWidthForKind`. Ordinary track remains one base unit; Sweep Left / Sweep Right use a longer 1.4× section and change heading by ±45°; Half-pipe and Wall ride are longer momentum-dependent stunt pieces. Because the ride already measures each rendered segment, the cart automatically spends the correct amount of route distance inside these longer pieces rather than relying on a fixed per-piece duration.
...
The coaster surface now has explicit Build and Ride modes. Build keeps the world primary, shows construction order only where useful, and puts Track and Park controls into secondary drawers. Every explored word-piece can use the full track palette. Ride hides the workshop, expands the coaster stage, reveals the draggable cart and gives launch tuning its own dock. The same local coaster state backs both modes; switching mode changes presentation and available interaction, not the underlying ride.
